...
create table users (
  ...
  current_team_id bigint references teams on delete
  set null
);
...
alter table teams_roles enable row level security;
-- Allow users to see which orgs they belong to
create policy "Can view own teams" on teams_users for
select using (auth.uid() = teams_users.user_id);
-- Allow users to read their own teams
create policy "Can view own teams" on teams for
select using (
    auth.uid() in (
      select user_id
      from teams_users
      where id = teams_users.team_id
    )
  );
-- Allow users to create teams
create policy "Can create teams" on teams for
